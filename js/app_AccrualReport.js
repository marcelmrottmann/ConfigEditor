...
function parseInputJson(data) {
	...
	function GETBS(x) {
		...
		var options3 = {
			'method': 'POST',
			'url': 'https://' + SelectedConnection.url + '/api/v1/timekeeping/timecard_metrics/multi_read',
			'headers': {
	...
	function RenderHandsOnTable(x) {
		...
		if (SelectedMapType == 'DS') {


			for (let i = 0, l = FinalArray[0].accrualSummaryData.length; i < l; i++) {
				g = FinalArray[0].accrualSummaryData[i]
				for (let z = 0, y = g.dailySummaries.length; z < y; z++) {
					if (g.accrualCode.typeName == 'Day')
					{
						WFCBalance = g.dailySummaries[z].currentBalance.vestedDaysAmount
						WFDAvailableBalance = g.dailySummaries[z].availableBalanceDays
						WFDEarnedToDate = g.dailySummaries[z].earnedToDateDays
						WFDTakenToDate = g.dailySummaries[z].takingToDateDays
						WFDPendingGrants = g.dailySummaries[z].pendingGrantDays
						WFDPlannedTakings = g.dailySummaries[z].plannedTakingDays
					}
					else if  (g.accrualCode.typeName == 'Money')
					{
						WFCBalance = g.dailySummaries[z].currentBalance.vestedMoneyAmount
						WFDAvailableBalance = g.dailySummaries[z].availableBalanceMoneyAmount
						WFDEarnedToDate = g.dailySummaries[z].earnedToDateMoneyAmount
						WFDTakenToDate = g.dailySummaries[z].takingToDateMoneyAmount
						WFDPendingGrants = g.dailySummaries[z].pendingGrantMoneyAmount
						WFDPlannedTakings = g.dailySummaries[z].plannedTakingMoneyAmount
					}
					else {
						WFCBalance = g.dailySummaries[z].currentBalance.vestedHoursAmount
						WFDAvailableBalance = g.dailySummaries[z].availableBalanceHours
						WFDEarnedToDate = g.dailySummaries[z].earnedToDateHours
						WFDTakenToDate = g.dailySummaries[z].takingToDateHours
						WFDPendingGrants = g.dailySummaries[z].pendingGrantHours
						WFDPlannedTakings = g.dailySummaries[z].plannedTakingHours
					}
					...
					map =
					{
						...
						"AccrualCode": g.accrualCode.name,
						"Accrual Type":g.accrualCode.typeName,
						"Balance Date":g.dailySummaries[z].balanceDate,
						"WFC Available Balance": WFCBalance,
						"WFD Available Balance":WFDAvailableBalance,
						"WFD Earned To Date":WFDEarnedToDate,
						"WFD Taken To Date":WFDTakenToDate,
						"WFD Pending Grants": WFDPendingGrants,
						"WFD Planned Takings":WFDPlannedTakings
			...
			for (let i = 0, l = FinalArray[0].accrualTransactions.length; i < l; i++) {
				g = FinalArray[0].accrualTransactions[i]
					...
					{
						Amount = g.daysAmount
						ProbationAmount = g.probationDaysAmount
						GrantAmount = g.grantDaysAmount
						LimitAmount = g.limitDaysAmount
						CarryforwardAmount = g.carryForwardDaysAmount
						OriginalAmount = g.originalDaysAmount
					}
					else if  (g.accrualCode.typeName == 'Money')
					{
						Amount = g.MoneyAmount
						ProbationAmount = g.probationMoneyAmount
						GrantAmount = g.grantMoneyAmount
						LimitAmount = g.limitMoneyAmount
						CarryforwardAmount = g.carryForwardMoneyAmount
						OriginalAmount = g.originalMoneyAmount
					}
					else {
						Amount = g.HoursAmount
						ProbationAmount = g.probationHoursAmount
						GrantAmount = g.grantHoursAmount
						LimitAmount = g.limitHoursAmount
						CarryforwardAmount = g.carryForwardHoursAmount
						OriginalAmount = g.originalHoursAmount
					}


					map =
					{
						"Employee": g.person.qualifier,
						"AccrualCode": g.accrualCode.name,
						"Accrual Type":g.accrualCode.typeName,
						...
						"Carryforward Amount":CarryforwardAmount ,
